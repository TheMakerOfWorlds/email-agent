...
It lets its owner use explicitly connected Gmail accounts through compact search, read, and send commands. Each account has a locally stored purpose note to help select the intended sender. This application is intended for personal use by its owner, not as a hosted email service.

## Privacy policy

Effective September 4, 2026.

**Access requested.** Email Agent requests Gmail read access (`gmail.readonly`) and Gmail send access (`gmail.send`). It does not request access to Google Drive, Calendar, Contacts, or Google account administration. Gmail read permission includes message content and the information Google exposes under that scope.

**Use of data.** The application searches and reads mail in response to the owner's requests and sends the messages the owner authorizes. It verifies the selected mailbox's identity and returns compact results. It does not operate advertising, resale, data-broker, or independent model-training functions.
