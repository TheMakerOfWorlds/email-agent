...
**Use of data.** The application searches and reads mail in response to the owner's requests sends the messages the owner authorizes, organizes selected mail and downloads chosen attachments, moves or restores mail for requested cleanup, and manages saved filters when authorized by the owner. Saved rules apply to future matching mail until removed. It verifies the selected mailbox's identity and returns compact results. It does not operate advertising, resale, data-broker, or independent model-training functions.
