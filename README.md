...
Eight everyday commands: `accounts`, `senders`, `search`, `read`, `send`, `status`, `trash`, and `restore`. The `doctor` command verifies authentication and cleanup permission; `scripts/auth.py` handles one-time setup.

## Gmail-only access

New connections request one Gmail permission:

- `https://www.googleapis.com/auth/gmail.modify` — read/search, send, move messages to Trash, and restore them.

No Drive, Calendar, Contacts, account administration, or broad `mail.google.com` permission. Gmail modify is the narrowest scope that supports Trash and restore; it does not allow immediate permanent message deletion. The client's HTTP allowlist further restricts writes to send, trash, and untrash. Legacy read/send grants remain usable until an account reconnects for cleanup; validation accepts only those Gmail scopes, including redundant legacy scopes Google may retain during an upgrade. Google defines the scopes in its [Gmail API documentation](https://developers.google.com/workspace/gmail/api/auth/scopes).
...
- Skill-based discovery: 27 tokens of name/description in the current synthetic measurement; the full 576-token instruction loads when relevant. No MCP tool schemas added.
